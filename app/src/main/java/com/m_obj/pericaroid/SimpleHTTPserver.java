...
public class SimpleHTTPserver
{
	...
	{
		return new Response( HTTP_OK, "text/plain", uri+"\r\n"+header.toString()+"\r\n"+parms.toString());
	}
		    ...
		    while(interfaces.hasMoreElements()){
		        ...
		        while(addresses.hasMoreElements()){
		            ...
		            if( !network.getName().equals("lo") && ia.getAddress().length==4 ){
		            	iplist.add( network.getName()+":"+ia.getHostAddress() );
		            }
		        }
		    }
	...
	public static String getIPAddress() {
		List<String> ipl = getIPAddressList();
		if(ipl.size()>0)
			return ipl.get(0).split(":")[1];
		return "";
	}
	**/
	/**
	...
	public class HTTPSession implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				// Ok, now do the serve()
				Response r = serve( uri, method, header, parms, fbuf, mySocket );
				if ( r == null )
